  ...
  describe CoreExt::HumanDateValues do
    describe "#leap_year?, #days_in_month, #days_in_year" do
      context "on a 4yr leap year" do
        subject { DateTime.new(2004, 2, 29) } # => Sun Feb 29 00:00:00 -0000 2004

        it "returns true for #leap_year?" do
          subject.should be_leap_year
        end

        it "reports the correct number of days in Feb" do
          subject.days_in_month.should == 29
        end

        it "reports the correct number of days in the year" do
          subject.days_in_year.should == 366
        end
      end

      context "on a 100yr leap year" do
        subject { DateTime.new(2100, 2, 28) } # => Sun Feb 28 00:00:00 -0000 2100

        it "returns true for #leap_year?" do
          subject.should_not be_leap_year
        end

        it "reports the correct number of days in Feb" do
          subject.days_in_month.should == 28
        end

        it "reports the correct number of days in the year" do
          subject.days_in_year.should == 365
        end
      end

      context "on a 400yr leap year" do
        subject { DateTime.new(2000, 2, 29) } # => Tue Feb 29 00:00:00 -0000 2000
