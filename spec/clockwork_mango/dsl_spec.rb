  ...
  describe "weekdays shortcuts (WEEKDAYS)" do
    ClockworkMango::Dsl::WEEKDAYS.each_with_index do |weekday, index|
      ...
      context "Dsl.#{weekday}" do
        ...
        it { predicate.should be_kind_of(ClockworkMango::EqualityPredicate) }
      ...
      it "should return a :wday EqualityPredicate with value #{value} when Dsl.#{weekday}s is called" do
        predicate = ClockworkMango::Dsl.send("#{weekday}s")
        predicate.should be_kind_of(ClockworkMango::EqualityPredicate)
        predicate.attribute.should == :wday
        predicate.value.should     == value
